...
export default function Index() {
    ...
    const postGenerateKey = (data) => {
        requestAPI('/rsa/dKeyGenNum', 'POST', data)
            .then(res => {
                if (res) {
    ...
    const postMessage = (data) => {
        ...
        console.log(msg);
        requestAPI('/rsa/rsaProcessNum', 'POST', msg)
            .then(res => {
                if (res) {
                    if (res.status === 500 || res.status === 405) {
    ...
    const postMessageE = (data) => {
        requestAPI('/rsa/rsaProcessNum', 'POST', data)
            .then(res => {
                if (res) {
                    if (res.status === 500 || res.status === 405) {
                        notificationCustom("Nhắc Nhở", `Gửi Tin Nhắn Thất Bại, Vui lòng thử lại`, "warning")
                ...
                <div className="blog__container-aside-wrapper">
                    <div onClick={() => setVisible(true)} style={{
                        ...
                        <div className="header__social">
                            <Link className="header__social-wrapper" to="/RSA">
                                <span className="header__social-wrapper-btn"><i class="far fa-arrow-alt-circle-left"></i></span>
                            </Link>
                        </div>
                        <ButtonCustom nameButton="Tạo Khóa & Gửi Message Số" icon="cloud-upload" />
